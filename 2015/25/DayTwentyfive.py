...
The weather machine beeps! On the console of the machine is a copy protection message asking you to enter a
code from the instruction manual. Apparently, it refuses to run unless you give it that code. No problem; you'll just look up the code in the--
...
Multiply it by 252533 to get 5088824049625. Then, divide that by 33554393, which leaves a remainder of 31916031. That remainder is the second code.
...
you're missing the one at 7,1 that would come before 6,2. But, it should be enough to let your-- oh, it's time for lunch! Bye!" The call disconnects.
...
def formateDocument(document: list[str]) -> list[int]:
    return [int(zahl) for zahl in re.findall(r'\d+', document[0])]
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2015/25/input.txt")


def part1(lines) -> int:
    side = lines[0] + lines[1] - 1
    previous_codes_count = (side * (side - 1)) // 2
    code_index = previous_codes_count + lines[1]

    code = 20151125
    multiplier = 252533
    divisor = 33554393

    factor = pow(multiplier, code_index - 1, divisor)
    result = (code * factor) % divisor
