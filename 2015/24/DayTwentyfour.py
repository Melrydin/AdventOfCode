...
needs as few packages as possible so that Santa has some legroom left over. It doesn't matter how many packages are in either of the other two groups,
...
def inputDocument(document: str) -> list[str]:
    with open(document, "r") as file:
        input = [int(line.strip()) for line in file.readlines() if line.strip()]
    return input
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2015/24/input.txt")


def findIdealConfiguration(packages):
    total_weight = sum(packages)
    if total_weight % 3 != 0:
        return None
    target_weight = total_weight // 3

    minLegroom = float('inf')
    min_qe = float('inf')

    # Try different sizes for group 1, starting from smallest
    for size in range(1, len(packages)-2):
        # Generate all possible combinations of given size
        for group1 in combinations(packages, size):
            if sum(group1) != target_weight:
                continue

            # Found a valid group1, check if remaining packages can be split evenly
            remaining = list(set(packages) - set(group1))

            # Try to find valid group2
            can_split = False
            for group2_size in range(1, len(remaining)-1):
                for group2 in combinations(remaining, group2_size):
                    if sum(group2) == target_weight:
                        group3 = list(set(remaining) - set(group2))
            ...
            if can_split:
                ...
                if size < minLegroom:
                    minLegroom = size
                    min_qe = math.prod(group1)
                elif size == minLegroom:
                    min_qe = min(min_qe, math.prod(group1))

        # If we found any valid configurations with this size, we're done
        if minLegroom == size:
            break
...
def findIdealConfiguration4(packages):
    total_weight = sum(packages)
    if total_weight % 4 != 0:
        return None
    ...
    for size in range(1, len(packages)-3):
        for group1 in combinations(packages, size):
            if sum(group1) != target_weight:
                continue

            remaining1 = list(set(packages) - set(group1))
            can_split = False

            for group2_size in range(1, len(remaining1)-2):
                for group2 in combinations(remaining1, group2_size):
                    if sum(group2) != target_weight:
                        continue

                    remaining2 = list(set(remaining1) - set(group2))

                    for group3_size in range(1, len(remaining2)-1):
                        for group3 in combinations(remaining2, group3_size):
                            if sum(group3) != target_weight:
                                continue

                            group4 = list(set(remaining2) - set(group3))
                            if sum(group4) == target_weight:
                                can_split = True
                                break
...
if __name__ == "__main__":
    document = testCase(1)
    print(f"Part 1: {findIdealConfiguration(document)}")
    print(f"Part 2: {findIdealConfiguration4(document)}")
