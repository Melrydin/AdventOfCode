...
So, in the example above, there are 4 distinct molecules (not five, because HOOH appears twice) after one replacement from HOH. Santa's favorite molecule, HOHOHO,
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2015/19/input.txt")


def formatDocument(document: list[str]) -> (list[tuple[str,str]], str):
    replacements = []
    medicine = document[-1]
    document = document[:-1]
    for line in document:
        fromMolecule, toMolecule = line.split(" => ")
        replacements.append((fromMolecule,toMolecule))
    return replacements, medicine


def CalculateDifferentMolecules(replacements: list[tuple[str,str]], medicine: str) -> int:
    molecule = set()
    for replacement in replacements:
        fromMolecule = replacement[0]
        toMolecule = replacement[1]
        for i in range(len(medicine)):
            if fromMolecule == medicine[i:i+len(fromMolecule)]:
                newMolecule = medicine[:i] + toMolecule + medicine[i+len(fromMolecule):]
                molecule.add(newMolecule)
    return len(molecule)


def findMinimumSteps(replacements, medicine):
    target = medicine
    counter = 0
    while target != 'e':
        tmp = target
...
if __name__ == "__main__":
    document = testCase(2)
    replacements, medicine = formatDocument(document)
    print(f"Part 1: {CalculateDifferentMolecules(replacements, medicine)}")
    print(f"Part 2: {findMinimumSteps(replacements, medicine)}")
