...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2015/01/input.txt")

def calculateFloor(instructions: list[str]) -> int:
    floor = 0
...
def enterABasement(instructions: list[str]) -> int:
    floor = 0
    position = 0
    for instruction in instructions[0]:
        position += 1
...
if __name__ == "__main__":
    instructions = testCase(1)
    print(f"Part 1: {calculateFloor(instructions)}")
    print(f"Part 2: {enterABasement(instructions)}")
