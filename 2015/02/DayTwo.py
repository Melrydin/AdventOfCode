...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2015/02/input.txt")


def formatDocument(document: list[str]) -> list[list[int]]:
    return [[int(dim) for dim in line.split("x")] for line in document]


def calculatePaper(dimensons: list[int]) -> int:
    l, w, h = dimensons
    return 2*l*w + 2*w*h + 2*h*l + min(l*w,l*h,w*h)


def calculateAll(dimensions: list[int]) -> int:
    paper = 0
    band = 0
    for box in dimensions:
        paper += calculatePaper(box)
        band += calculatBand(box)
    return paper, band


def calculatBand(dimensions: list[int]) -> int:
    l, w, h = dimensions
    dimensions = sorted(dimensions)
    return (dimensions[0]+dimensions[0]+dimensions[1]+dimensions[1]) + (l*w*h)


if __name__ == "__main__":
    document = testCase(1)
    format = formatDocument(document)
    paper, band = calculateAll(format)
