...
However, it is important to realize the difference between the number of characters in the code representation of the string literal and
...
--- Part Two ---
Now, let's go the other way. In addition to finding the number of characters of code, you should now encode each code representation as a new string and find the number of characters of the new encoded representation,
...
def testCase(test: int = 0):
    if test == 0:
        return inputDocument("2015/08/test.txt")
    else:
        return inputDocument("2015/08/input.txt")


def countDifference(document: list[str]) -> int:
    totalCodeChars = 0
    totalMemoryChars = 0

    for line in document:
        codeChars = len(line.encode('utf-8'))
        memoryChars = len(bytes(line, "utf-8").decode("unicode_escape")) - 2
...
def countDifferenceEncoded(document: list[str]) -> int:
    totalCodeChars = 0
    totalEncodedChars = 0

    for line in document:
        codeChars = len(line.encode('utf-8'))
        encodedChars = len(f'"{line.encode("unicode_escape").decode("utf-8")}"'.encode('utf-8')) + 2
        if '"' in line[1:-1]:
            encodedChars += (line[1:-1].count('"'))
        totalCodeChars += codeChars
...
if __name__ == "__main__":
    document = testCase(1)
    print(f"Part 1: {countDifference(document)}")
    print(f"Part 2: {countDifferenceEncoded(document)}")
