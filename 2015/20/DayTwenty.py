...
def housePresents(presentsCount: int) -> int:
    house = 1
    while True:
        totalPresents = 0
        # Only check divisors up to sqrt(houseNumber) for efficiency
        for elf in range(1, int(house ** 0.5) + 1):
            if house % elf == 0:
                ...
                pair = house // elf
                if pair != elf:
                    totalPresents += pair * 10

            # Early termination if we've exceeded target
            if totalPresents >= presentsCount:
                return house

        house += 1


def housePresentsWith50Steps(presentsCount: int) -> int:
    house = 1
    while True:
        totalPresents = 0
        # Only check divisors up to sqrt(houseNumber) for efficiency
        for elf in range(1, int(house ** 0.5) + 1):
            if house % elf == 0:
                # Check if within 50 houses for current elf
                if house <= elf * 50:
                    totalPresents += elf * 11
                # Check pair divisor
                pair = house // elf
                if pair != elf and house <= pair * 50:
                    totalPresents += pair * 11
...
if __name__ == "__main__":
    presents = testCase(1)
    print(f"Part 1: {housePresents(presents)}")
    print(f"Part 2: {housePresentsWith50Steps(presents)}")
