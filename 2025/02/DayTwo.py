...
As it turns out, one of the younger Elves was playing on a gift shop computer and managed to add a whole bunch
of invalid product IDs to their gift shop database! Surely, it would be no trouble for you to identify the invalid product IDs for them, right?
...
def formateDocument(document: list[str]) -> list[tuple[int,int]]:
    return [(numberRange.split("-")[0], numberRange.split("-")[1]) for numberRange in document]
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2025/02/input.txt")


def part1(numbers: list[tuple]) -> int:
    sumOfInvalidNumbers = 0
    for numberRange in numbers:
        start = numberRange[0]
        end = numberRange[1]

        for number in range(int(start), int(end)+1):
            numberToString = str(number)
            lenght = len(numberToString)

            if lenght % 2 == 0:
                mitte = lenght // 2
                firstHalf = numberToString[:mitte]
                twoHalf = numberToString[mitte:]

                if firstHalf == twoHalf:
                    sumOfInvalidNumbers += number

    return sumOfInvalidNumbers


def part2(numbers: list[tuple]) -> int:
    sumOfInvalidNumbers = 0
