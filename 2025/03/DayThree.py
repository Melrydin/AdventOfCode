...
The batteries are each labeled with their joltage rating, a value from 1 to 9. You make a note of their joltage ratings (your puzzle input). For example:
...
you need to turn on exactly two batteries; the joltage that the bank produces is equal to the number formed by the digits on the batteries you've turned on.
...
of the system and hits the big red "joltage limit safety override" button. You lose count of the number of times she needs to confirm "yes,
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2025/03/input.txt")
...
def part1(numbers: list[str]) -> int:
    ...
    for number in numbers:
        ...
        batteryTwo = 0
        for num in number[:-1]:
            if int(num) > int(batteryOne):
                batteryOne = num
        for num in number[number.find(batteryOne)+1:]:
            if int(num) > int(batteryTwo):
                batteryTwo = num
        sumJoltage += int(batteryOne + batteryTwo)
    return sumJoltage
...
def part2(numbers: list[str]) -> int:
    ...
    TAGET_LENGTH = 12
    for number in numbers:
        toRemove = len(number) - TAGET_LENGTH
        stack = []
        for digit in number:
            while toRemove > 0 and stack and stack[-1] < digit:
                stack.pop()
                toRemove -= 1
            stack.append(digit)
        result = "".join(stack[:TAGET_LENGTH])
        sumJoltage += int(result)
    return sumJoltage
