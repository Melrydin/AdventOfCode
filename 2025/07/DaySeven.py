...
the only way out is the teleporter. Unfortunately, the teleporter seems to be leaking magic smoke.

Since this is a teleporter lab, there are lots of spare parts, manuals,
and diagnostic equipment lying around. After connecting one of the diagnostic tools,
it helpfully displays error code 0H-N0, which apparently means that there's an issue with one of the tachyon manifolds.
...
each time a particle reaches a splitter, it's actually time itself which splits. In one timeline,
...
active after a single particle completes all of its possible journeys through the manifold.
In the above example, there are many timelines. For instance,
there's the timeline where the particle always went left:
...
def formateDocument(document: list[str]):
    temp = [list(line) for line in document]
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2025/07/input.txt")
...
def part1(grid: np.ndarray) -> int:
    ...
    grid[1][np.where(grid[0] == "S")[0][0]] = "|"

    for row in range(1,rows):
        flow_in = (grid[row-1] == '|')
        current_row_dots = (grid[row] == '.')
        current_row_splitters = (grid[row] == '^')
        active_splitters = flow_in & current_row_splitters

        if np.any(active_splitters):
            mask_left = active_splitters[1:] & current_row_dots[:-1]
            mask_right = active_splitters[:-1] & current_row_dots[1:]

            success_left = np.zeros(cols, dtype=bool)
            success_left[1:] = mask_left
            success_right = np.zeros(cols, dtype=bool)
            success_right[:-1] = mask_right

            split_count += np.sum(success_left | success_right)
            grid[row, :-1][mask_left] = '|'
            grid[row, 1:][mask_right] = '|'

        to_fill = flow_in & current_row_dots
        grid[row, to_fill] = '|'
    return split_count
...
def part2(grid: np.ndarray) -> int:
    ...
    start_pos = np.where(grid[0] == 'S')[0][0]
    counts[1, start_pos] = 1

    for row in range(1,rows):
        incoming = counts[row-1]

        current_dots = (grid[row] == '.')
        current_splitters = (grid[row] == '^')

        counts[row] += incoming * current_dots
        active_splitter_counts = incoming * current_splitters

        if np.any(active_splitter_counts > 0):
            target_is_dot_left = current_dots[:-1]
            counts[row, :-1] += active_splitter_counts[1:] * target_is_dot_left

            target_is_dot_right = current_dots[1:]
            counts[row, 1:] += active_splitter_counts[:-1] * target_is_dot_right
