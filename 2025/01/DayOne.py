...
def formateDocument(document: list[str]) -> list[tuple]:
    return [-int(dial[1:]) if dial[0] == "L" else int(dial[1:]) for dial in document]
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2025/01/input.txt")


def part1(dials: list[int]) -> int:
    position = 50
    zeros = 0
    for dial in dials:
        position = (position + dial) % 100
        if position == 0: zeros += 1
    return zeros

def part2(dials: list[int]) -> int:
    position = 50
    zeros = 0
    for dial in dials:
        for _ in range(abs(dial)):
            if dial < 0:
