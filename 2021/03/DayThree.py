...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2021/03/input.txt")


def part1(lines: np.ndarray) -> int:
    rows, cols = lines.shape
    ones_count = (lines == '1').sum(axis=0)
    gamma_bits = (ones_count > (rows / 2)).astype(int)
    gamma_string = "".join(gamma_bits.astype(str))
...
def part2(lines: np.ndarray) -> int:
    oxygen_rating = get_rating(lines, most_common=True)
    co2_rating = get_rating(lines, most_common=False)
    return oxygen_rating * co2_rating


def get_rating(data: np.ndarray, most_common: bool) -> int:
    candidates = data
    _, cols = candidates.shape
    ...
    for i in range(cols):
        ...
        col = candidates[:, i]
        count_1 = np.count_nonzero(col == '1')
        count_0 = len(col) - count_1

        if most_common:
            target_bit = '1' if count_1 >= count_0 else '0'
        else:
            target_bit = '0' if count_1 >= count_0 else '1'

        candidates = candidates[col == target_bit]
    result_string = "".join(candidates[0])
