...
It's covered in Christmas lights (because of course it is), and it even has an experimental antenna that should
be able to track the keys if you can boost its signal strength high enough;
...
def formateDocument(document: list[str]):
    return [int(line) for line in document]
...
def testCase(test: int = 0):
    if test == 0:
        ...
        return inputDocument("2021/01/input.txt")
...
def part2(input) -> int:
    ...
    for i in range(0,len(input)-3):
        numberOne = input[i] + input[i+1] + input[i+2]
        numberTwo = input[i+1] + input[i+2] + input[i+3]
        if numberOne < numberTwo:
            increases_counter += 1
    return increases_counter
