...
def movetheGurd(document: list[str]) -> int:
    ...
    y = document[x].index("^")
    gurdAHead = "^"
    document = [list(row) for row in document]  # Convert document into a mutable list of lists
    path = [(x, y)]

    while 0 < x < len(document) - 1 and 0 < y < len(document[0]) - 1:
        # Move up ^
        if gurdAHead == "^":
            if document[x - 1][y] == "#":
                gurdAHead = ">"
...
if __name__ == "__main__":
    document = testCase(0)
    steps, path = movetheGurd(document)
    print(path)
