...
def formating(input: list):
    ...
    for position in input:
        ...
        currentPosition.update({position[0]: position[1].replace("(", "").replace(")", "").split(", ")})
...
def findPositionMoveCount(instructions: list, PositionMove: dict):
    ...
    while currentPosition != "ZZZ":
        if counter == len(instructions):
            instructions = instructions + groundstring
        if instructions[counter] == "R":
            currentPosition = PositionMove.get(currentPosition)[1]
        else:
            currentPosition = PositionMove.get(currentPosition)[0]
        counter += 1
    return counter
...
def testCase(part: int = 0):
    if part == 0:
        ...
        return inputDocument("2023/08/DayEight.txt")
