...
def calculateHashSume(hash: str):
    hashSume = 0
    for symbol in hash:
        hashSume += ord(symbol)
        hashSume *= 17
...
def sumeOfAllHashes(input: list):
    sume = 0
    for hash in input:
        sume += calculateHashSume(hash)
    return sume
...
def testCase(part: int= 0):
    if part == 0:
        ...
        return inputDocument("2023/15/DayFivteen.txt")
    

if __name__ == "__main__":
    input = testCase(1)
    print("Part 1: {}".format(sumeOfAllHashes(input)))